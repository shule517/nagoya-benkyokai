class SearchEventService
  ...
  def call(search_condition, after_today = true)
    @search_condition = search_condition.merge(keyword: search_keywords)
  ...
  def aichi?(event)
    ...
    aichi_cities.any? { |place| event.address&.include?(place) } &&
    not_aichi_cities.all? { |ng_place| event.address&.exclude?(ng_place) }
  end

  def benkyokai?(event)
    not_benkyokai_words.all? { |not_benkyokai_word| event.title.exclude?(not_benkyokai_word) }
  end

  def search_keywords # 検索ワード(愛知・中部・東海・全市町村)
  ...
  def not_benkyokai_words # NGワード(勉強会ではない)
  ...
  def aichi_cities # 愛知県内(全市町村・名古屋の全区)
