...
describe ConnpassScraping, type: :request do
  ...
  context 'JXUGC #14 Xamarinの場合' do
    ...
    it 'イベント情報が取得できること', vcr: '#find-event' do
      ...
      expect(event.catch).to match /\Aにゃごやでも話題の Xamarin を触ってみよう！<br>こんにちは。エクセルソフトの田淵です。 今話題の Xamarin を名古屋でも触ってみましょう！/
  ...
  describe '#catch', vcr: '#catch' do
    ...
    it 'キャッチにイベント詳細が結合されていること' do
      expect(event.catch).to match /\Aゼロから作る Deep Learning 読書会＋ハンズオン その2<br>機械学習 名古屋 分科会 機械学習名古屋 勉強会の分科会です。 この分科会では、より理論・実装に重きを置いた勉強をしていきます。/
    end
  end
end
