...
describe SearchEventService, type: :request do
  it '全てのサイトからイベントが取得されていること', vcr: '#search-all' do
    events = SearchEventService.new.search({ ym: '201612' }, false)
  ...
  it '2017/05のDoorkeeperのイベントが取得できること', vcr: '#search-201705' do
    events = SearchEventService.new.search({ ym: '201705' }, false)
    titles = events.map(&:title)
  ...
  context 'connpassの場合' do
    it '勉強会ではないイベントは除外すること', vcr: 'connpass_not_benkyokai' do
      events = SearchEventService.new.search({ event_id: 56804 }, false) # 名古屋手帳オフ会 https://758techo-bu.connpass.com/event/56804/
      titles = events.map(&:title)
  ...
  context 'Doorkeeperの場合' do
    it '勉強会ではないイベントは除外すること', vcr: 'doorkeeper_not_benkyokai' do
      events = SearchEventService.new.search({ event_id: 56389 }, false) # mana×comu　番外編　「働く女性の癒しヨガ」 https://manacomu.doorkeeper.jp/events/56389
      titles = events.map(&:title)
  ...
  context 'ATNDの場合' do
    ...
    it '勉強会ではないイベントは除外すること', vcr: 'atnd_not_benkyokai' do
      events = SearchEventService.new.search({ ym: ['201705', '201706'] }, false)
      titles = events.map(&:title)
  ...
  context '開催場所に名古屋が入ってない場合', vcr: '#find-JPSPS' do
    ...
    let(:events) { SearchEventService.new.search({ event_id: 47375 }, false) }
